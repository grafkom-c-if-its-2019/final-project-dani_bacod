...
export const generateMap = (mapArr) => {
  ...
  //*----- WALL -----*/
  const wallTexture = new THREE.TextureLoader().load('images/plaster.jpg');
  ...
  const halfExtentswall = new CANNON.Vec3(2, 3, 2);
  const fixedCubeShape = new CANNON.Box(halfExtents);
  const fixedCubewall = new CANNON.Box(halfExtentswall);
  const fixedCubeGeometry = new THREE.BoxGeometry(halfExtents.x * 2, 3.8, halfExtents.z * 2);


  const wallGeometry = new THREE.BoxGeometry(halfExtentswall.x * 2, 6, halfExtentswall.z * 2);
  ...
  for (let j = 0; j < mapArrWidth; j++) {
    for (let k = 0; k < mapArrHeight; k++) {
      ...
      if (mapArr[j][k] === 4) { // CREATE TREE
        ...
        const wall = new Wall(wallMaterial, wallTexture, fixedCubewall, wallGeometry, x, y, z);
        wall.init();
